...
function renderSuppliers(suppliers) {
  ...
  if (!Array.isArray(suppliers) || suppliers.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="8" class="empty-state">No suppliers found. Click "Add Supplier" to create one.</td></tr>';
    return;
  }
  ...
  tbody.innerHTML = suppliers
    .map((s) => {
      const cats = Array.isArray(s.categories)
        ...
        : s.categories || "";
      const rating =
        s.rating === undefined || s.rating === null || s.rating === ""
          ? 3
          : s.rating;
...
async function editSupplier(id) {
  try {
    ...
    document.getElementById("categories").value = Array.isArray(
      supplier.categories,
    )
